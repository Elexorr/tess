...
def deno():
    global ffi_lc, tpf_lc, search
    if search == 'ffi':
        cbvCorrector = CBVCorrector(ffi_lc, interpolate_cbvs=True, extrapolate_cbvs=True)
        sets = str(cbvCorrector.cbvs)
        # Odstránenie hranatých zátvoriek na začiatku a na konci reťazca
        sets = sets.strip("[]")
        # Rozdelenie reťazca na časti podľa "TESS CBVs"
        parts = sets.split("TESS CBVs")
        # Iterovanie a výpis každej časti s pridaním "TESS CBVs" na začiatok
        for part in parts[1:]:  # Preskočíme prvý prázdny prvok
            # print(f"TESS CBVs{part.strip()}")
            T.insert(INSERT, f"TESS CBVs{part.strip()}"'\n')
        T.see(tk.END)

deno_button = ttk.Button(frame1, text='Denoise', command=deno)
deno_button.place(x=200, y=150)
