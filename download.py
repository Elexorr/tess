...
with open('kepler.csv', newline='') as csvfile:  # Otvori a nacita
    ...
    for row in reader:  # Ide po KIC-kach riadok
        ...
        if len(row['#KIC']) == 7:  #
            url = "http://keplerebs.villanova.edu/includes/" + "0" + row['#KIC'] + ".00.lc.pf.png"  # Vytvori url na stiahnutie
        else:  # obrazku dtr krivky
            url = "http://keplerebs.villanova.edu/includes/" + row['#KIC'] + ".00.lc.pf.png"  #
        ...
        Im = ImageDraw.Draw(i)
        mf = ImageFont.truetype('times.ttf', size = 24)
        Im.text((5, 5), target_name, (0, 0, 0), font = mf)
        i.save(completeName)